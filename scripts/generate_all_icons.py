...
def process_logo(src_path, dest_path, dest_size=None, zoom_factor=1.85, fill_bg=True, logo_scale=0.85):
    try:
        ...
        for item in datas:
            r, g, b, a = item
            # Quitar blanco (con más tolerancia para grises claros) y el azul oscuro viejo
            if (r > 190 and g > 190 and b > 190) or (r < 80 and g < 90 and b < 110):
...
jobs = [
    ...
    # Iconos PWA (Sólidos para evitar el marco blanco que pone Android)
    (SRC_LOGO, os.path.join(PUBLIC_DIR, "icon-192-v18.png"), (192, 192), True, 0.82),
    (SRC_LOGO, os.path.join(PUBLIC_DIR, "icon-512-v18.png"), (512, 512), True, 0.82),
    
    # Adaptive / Maskable (Sólidos para que el icono llene toda la forma)
    (SRC_LOGO, os.path.join(PUBLIC_DIR, "maskable-192-v18.png"), (192, 192), True, 0.75),
    (SRC_LOGO, os.path.join(PUBLIC_DIR, "maskable-512-v18.png"), (512, 512), True, 0.75)
]

if __name__ == "__main__":
    print("🚀 Generando iconos CarMatch v18 (ULTRA CACHE BUSTER)...")
    for src, dest, size, fill, scale in jobs:
        process_logo(src, dest, dest_size=size, fill_bg=fill, logo_scale=sca