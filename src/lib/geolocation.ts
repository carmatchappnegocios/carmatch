...
export async function getLocationFromIP(): Promise<LocationData> {
    try {
        ...
        console.warn('⚠️ Detection IP falló. Usando fallback neutro...')
        // Retornamos null o dejamos que el capturador maneje el error
        // para evitar posicionar falsamente en Monterrey
        throw error
    }
}
...
/**
 * Calcula el bounding box para una ubicación y radio dados
 * @param lat Latitud central
 * @param lng Longitud central
 * @param radiusKm Radio en kilómetros
 * @returns Objeto con minLat, maxLat, minLng, maxLng
 */
