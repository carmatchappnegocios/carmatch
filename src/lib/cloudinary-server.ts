 ...
 */
export function applyLogoOverlay(imagePublicId: string, placement?: string): string {
    ...
    if (placement) {
        ...
        const logo = `l_${logoPublicId.replace(/\//g, ':')},w_0.20,o_75`;
        
        switch(p) {
            case 'TOP_LEFT': transform = `${logo},g_north_west,x_50,y_50`; break;
            case 'TOP_RIGHT': transform = `${logo},g_north_east,x_50,y_50`; break;
            case 'BOTTOM_LEFT': transform = `${logo},g_south_west,x_50,y_50`; break;
            case 'BOTTOM_RIGHT': transform = `${logo},g_south_east,x_50,y_50`; break;
            case 'CENTER_TOP': transform = `${logo},g_north,y_70`; break;
            case 'CENTER_BOTTOM': transform = `${logo},g_south,y_70`; break;
            default: transform = `${logo},g_south_east,x_50,y_50`;
        }
    } else {
        // Legacy: Random placement strategy for variety
        const strategies = [
            `l_${logoPublicId.replace(/\//g, ':')},w_0.20,o_70,g_south_east,x_40,y_40`,
            `l_${logoPublicId.replace(/\//g, ':')},w_0.18,o_75,g_south_west,x_30,y_35`,
            `l_${logoPublicId.replace(/\//g, ':')},w_0.22,o_65,g_north_west,x_30,y_30`,
            `l_${logoPublicId.replace(/\//g, ':')},w_0.20,o_70,g_north_east,x_40,y_40`,
        ];
        transform = strategies[Math.floor(Math.random() * strategies.length)];
    }
    
    return `https://res.cloudinary.com/${cloudName}/image/upload/${transform}/${imagePublicId}`;
}
