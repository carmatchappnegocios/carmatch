...
export default function MarketFiltersAdvanced({
    ...
    // 📍 SYNC LOCATION INPUT SOLO SI CAMBIA EXTERNAMENTE EL CONTEXTO
    const lastContextLocation = useRef(manualLocation?.city || location?.city)

    useEffect(() => {
        const currentContextCity = manualLocation?.city || location?.city;
        if (currentContextCity && currentContextCity !== lastContextLocation.current) {
            lastContextLocation.current = currentContextCity;
            ...
            <div className="mb-4">
                <AIChatWidget context="market"
                    ...
                    onFilterChange={(filters) => {
                        ...
                        if (filters.query) params.set('search', filters.query) // 🔍 FIX: Pass raw query to server
