...
export default function SOSComponent({ isActive, otherUserId, onEndMeeting, chatId, activeAppointmentId, trustedContact }: SOSComponentProps) {
    ...
    const triggerSOS = async () => {
        ...
        // 🛡️ [ZENITH AUDIT] FAIL-SAFE: Definir coordenadas iniciales seguras (0,0 es mejor que nada)
        let currentLat = 0;
        let currentLng = 0;

        try {
            // 🔥 FIX: Intentar capturar ubicación REAL con timeout estricto
            if (navigator.geolocation) {
                try {
                    const pos = await new Promise<GeolocationPosition>((resolve, reject) => {
                        navigator.geolocation.getCurrentPosition(resolve, reject, { 
                            enableHighAccuracy: true,
                            timeout: 4000 // Reducido a 4s para no retrasar la alerta vital
                        });
                    });
                    currentLat = pos.coords.latitude;
                    currentLng = pos.coords.longitude;
                    console.log("✅ GPS capturado con éxito para SOS:", currentLat, currentLng);
                } catch (gpsError) {
                    console.warn("⚠️ GPS falló o tiempo de espera agotado, enviando SOS sin coordenadas precisas.", gpsError);
                }
            }

            // 🚀 ESTO ES VITAL: Se envía la alerta SIEMPRE, incluso si GPS es 0,0
            const sosResponse = await fetch(`/api/chats/${chatId}/sos`, {
                ...
                body: JSON.stringify({
                    ...
                    isTest: (window as any).isSOSTestMode
                })
            })
            ...
            if (sosResponse.ok) {
                ...
                if (sosData.alertId) {
                    ...
                    return; // Detener flujo para evitar cargar mapa innecesario
                }
            }

            // Intentar cargar ubicación de la contraparte solo si la alerta base ya se procesó
            ...
            <div className="sticky top-0 left-0 right-0 bg-red-600 text-white px-4 py-2 flex items-center justify-between z-40 shadow-lg animate-pulse-slow">
                <div className="flex items-center gap-2">
                    <div className="relative">
                        ...
                        <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-green-400 rounded-full border-2 border-red-600 animate-ping"></span>
                    </div>
                    <div>
