...
export default function BusinessDetailsModal({ business, onClose, categoryColor = '#0ea5e9', categoryEmoji = '🔧' }: BusinessDetailsModalProps) {
    ...
    return (
        <div className="fixed inset-0 top-[68px] z-[10000] flex flex-col items-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in overflow-y-auto" onClick={onClose}>
            {/* ❌ Close Button */}
            <button
                onClick={onClose}
            ...
            >
                ...
                <div className="p-6 md:p-8 space-y-8 pb-32">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        {/* Additional Phones */}
                        {business.additionalPhones && business.additionalPhones.map((ap, idx) => {
                            const cleanNumber = ap.replace(/\D/g, '');
                            if (!cleanNumber) return null;
                            return (
                                <a
                                    key={`ap-${idx}`}
                                    href={`tel:${cleanNumber}`}
                                    className="col-span-2 sm:col-span-1 bg-surface-highlight/50 hover:bg-surface-highlight/80 text-text-primary border border-white/5 py-3 px-4 rounded-xl font-bold flex items-center justify-center gap-2 transition"
                                >
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                    {t('business_details.call')} {idx + 2}
                                </a>
                            );
                        })}
                        ...
                        {(business.facebook || business.instagram || business.tiktok) && (
                            <div className="col-span-2 flex gap-2">
                                {business.facebook && (
                                    <a href={business.facebook} target="_blank" rel="noopener noreferrer" className="flex-1 bg-[#1877F2]/10 text-[#1877F2] hover:bg-[#1877F2]/20 py-3 rounded-xl flex items-center justify-center transition">
                                        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                                    </a>
                                )}
                                {business.instagram && (
                                    <a href={business.instagram} target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] text-white hover:opacity-90 py-3 rounded-xl flex items-center justify-center transition shadow-lg shadow-pink-900/20" title="Instagram">
                                        <Instagram className="w-6 h-6" />
                                    </a>
                                )}
                                {business.tiktok && (
                                    <a href={business.tiktok} target="_blank" rel="noopener noreferrer" className="flex-1 bg-black hover:bg-gray-900 text-white border border-white/10 py-3 rounded-xl flex items-center justify-center transition shadow-lg relative overflow-hidden group/tiktok">
                    ...
                    {/* 🛡️ Report Button */}
                    <div className="mt-12 pt-8 border-t border-surface-highlight/50 text-center">
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                setShowReportModal(true);
                            }}
                            className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-text-secondary hover:text-red-500 transition-colors py-2 px-4 rounded-xl border border-transparent hover:border-red-500/20 hover:bg-red-500/5"
                        >
                            <AlertTriangle size={14} />
                            Reportar problema con este negocio
                        </button>
                    </div>
