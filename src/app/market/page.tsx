...
export async function generateMetadata({ searchParams }: { searchParams: Promise<SearchParams> }): Promise<Metadata> {
    ...
    // 🏷️ Construcción de Título Dinámico (SEO Master)
    let title = "Comprar Autos Usados en México | Marketplace CarMatch"
    let description = "Explora la mayor variedad de vehículos: autos, motos, tractores y maquinaria pesada con trato directo y sin comisiones en CarMatch."
    
    if (s.category) {
        title = `Venta de ${s.category === 'Motocicleta' ? 'Motos y Cuatrimotos' : s.category} | CarMatch Market`
        description = `Encuentra las mejores ofertas en ${s.category}. Compraventa directa sin intermediarios de ${s.category === 'Motocicleta' ? 'motos, cuatrimotos y bikers' : s.category}.`
    }
    
    if (s.brand) {
        title = `${s.brand} en Venta | Modelos Seminuevos y Nuevos | CarMatch`
        description = `Busca tu próximo ${s.brand}. Contamos con una amplia selección de modelos ${s.brand} en excelentes condiciones y con trato directo.`
    }
    
    if (s.search) {
        title = `Resultados para "${s.search}" | CarMatch Marketplace`
    }

    const keywords = [
        "vender mi auto gratis", "marketplace autos mexico", "compra venta vehiculos",
        "motos usadas", "cuatrimotos 4x4", "rzr en venta", "tractores usados", "maquinaria pesada",
        "seminuevos", "hondita", "vocho", "trocas"
    ]
...
export default async function MarketPage({
    ...
    // 🔥 DIGITAL BORDER CONTROL: Filtrar por país del usuario con tolerancia a variantes (MX, México)
    const normalizedCountry = currentUser?.country?.toLowerCase() || 'mexico'
    const isMex = ['mexico', 'méxico', 'mx'].includes(normalizedCountry)
    ...
    if (searchParams.search && searchParams.search.trim().length > 3) {
        try {
            ...
            let aiFilters: any = typeof searchCache.get === 'function' ? searchCache.get(searchParams.search) : undefined

            if (!aiFilters) {
                // 💰 PASO 2: Llamar a IA solo si no está en caché
                const userCity = searchParams.city || ""
                aiFilters = await interpretSearchQuery(searchParams.search, 'MARKET', userCity)
                // 💰 PASO 3: Guardar en caché
                searchCache.set(searchParams.search, aiFilters)
                console.log(`🤖 [AI Search] Nueva búsqueda: "${searchParams.search}" en ${userCity || 'Global'}`)
            } else {
            ...
            if (aiFilters.city && !searchParams.city) {
                where.city = { contains: aiFilters.city, mode: 'insensitive' }
            }
        ...
        <>
            ...
            <script dangerouslySetInnerHTML={{ __html: `
                if (/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
                    ...
                    if (vp) vp.content = 'width=device-width, initial-scale=1, maximum-scale=5, viewport-fit=cover';
