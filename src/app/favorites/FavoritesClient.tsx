...
import { Check, X, ShieldCheck, Zap, Info, BarChart2, Plus, ArrowRight } from 'lucide-react'
...
export default function FavoritesClient({ favorites }: { favorites: FavoriteVehicle[] }) {
    ...
    const toggleSelection = (e: React.MouseEvent, vehicleId: string) => {
        e.preventDefault()
        e.stopPropagation()
        setSelectedIds(prev => {
    ...
    const handleCompare = () => {
        ...
        const selectedVehicles = favorites
            .filter(f => selectedIds.includes(f.vehicle.id))
            .map(f => f.vehicle)

        // Generate slug pattern: brand-model-id1-vs-brand-model-id2
        // Since we want up to 3, let's use a simpler slug for IDs or follow the existing pattern
        // The existing pattern in comparar/[slug]/page.tsx uses slugA-vs-slugB
        // I will update comparar/[slug]/page.tsx to handle multi-vs
        
        const slugs = selectedVehicles.map(v => 
            `${v.brand.toLowerCase()}-${v.model.toLowerCase()}-${v.id}`
        )
        
        const comparisonSlug = slugs.join('-vs-')
        router.push(`/comparar/${comparisonSlug}`)
    }
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center mb-6">
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all font-bold text-sm ${isCompareMode 
                                    ? 'bg-primary-500 border-primary-500 text-white shadow-lg shadow-primary-500/20' 
                                    : 'bg-surface border-surface-highlight text-text-secondary hover:border-primary-500/50'
                                }`}
                            >
                                <BarChart2 size={18} />
                        ...
                        {favorites.map(({ vehicle }) => {
                            ...
                            return (
                                <div
                                    key={vehicle.id}
                                    onClick={(e) => isCompareMode ? toggleSelection(e, vehicle.id) : null}
                                    className={`bg-surface rounded-xl border transition shadow-lg group block relative cursor-pointer ${
                                        selectedIds.includes(vehicle.id) ? 'border-primary-500 bg-primary-500/5 ring-2 ring-primary-500/20' : 'border-surface-highlight hover:border-primary-500/50'
                                    } ${!isActive ? 'opacity-75 grayscale-[0.5]' : ''}`}
                                >
                                    <Link href={`/vehicle/${vehicle.id}`} className={isCompareMode ? 'pointer-events-none' : ''}>
                                        {/* Imagen del vehículo */}
                                        <div className="aspect-video bg-gradient-to-br from-surface-highlight to-surface flex items-center justify-center relative overflow-hidden">
                                            ...
                                            {/* Botón de Eliminar (Trash) - Only when NOT in compare mode */}
                                            {!isCompareMode && (
                                                <div className="absolute top-3 left-3 z-30">
                                            <button
            ...
            <AnimatePresence>
                {selectedIds.length > 0 && (
                    <motion.div 
                        ...
                        className="fixed bottom-24 inset-x-4 z-[100] max-w-2xl mx-auto"
                    >
                        <div className="bg-surface/80 backdrop-blur-xl border border-primary-500/30 rounded-2xl p-4 shadow-2xl flex items-center justify-between gap-4">
