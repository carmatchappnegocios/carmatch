...
export default function MyBusinessesClient() {
    ...
    const fetchBusinesses = async () => {
        try {
            ...
            // 🎯 Scroll to specific business if requested
            const targetId = searchParams.get('businessId')
            if (targetId) {
                setTimeout(() => {
                    const element = document.getElementById(`business-${targetId}`)
                    if (element) {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {loading ? (
                            ...
                            businesses.map(business => {
                                const isTarget = searchParams.get('businessId') === business.id
                                return (
                                    <div 
                                        key={business.id} 
                                        id={`business-${business.id}`}
                                        className={`rounded-xl border overflow-hidden transition-all duration-500 group ${business.status === 'ACTIVE'
