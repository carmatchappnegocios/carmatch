...
const MapBoxStoreLocator = dynamic(() => import('@/components/MapBoxStoreLocator'), {
    ssr: false,
    loading: () => <div className="w-full h-full bg-slate-900 flex items-center justify-center"><div className="flex flex-col items-center gap-3"><div className="w-10 h-10 border-4 border-primary-500 border-t-transparent rounded-full animate-spin"></div><span className="text-sm text-white/40 uppercase tracking-widest font-black">{"Loading 3D map..."}</span></div></div>
})
...
export default function MapClient({ businesses, user }: MapClientProps) {
    ...
    const memoizedLocation = useMemo(() => {
        if (location && (location.latitude !== 0 || location.longitude !== 0)) {
            return { latitude: location.latitude, longitude: location.longitude, source: location.source, accuracy: location.accuracy }
        }
        return undefined
    }, [location?.latitude, location?.longitude, location?.source, location?.accuracy])
