...
export default function ChatPage({
    ...
    const targetAppointmentId = searchParams?.appointmentId
    const { t } = useLanguage()
    ...
    const fetchMessages = async () => {
        try {
            ...
            setLoading(false)
            // 🎯 Scroll to specific appointment if requested
            if (targetAppointmentId) {
                setTimeout(() => {
                    const element = document.getElementById(`appointment-${targetAppointmentId}`)
                    if (element) {
                        element.scrollIntoView({ behavior: 'smooth', block: 'center' })
                        element.classList.add('ring-2', 'ring-primary-500', 'ring-offset-2', 'ring-offset-background', 'animate-pulse')
                        ...
                        <div className="max-w-4xl mx-auto space-y-4">
                            ...
                            {messages.length === 0 ? (
                                ...
                                if (message.type === 'APPOINTMENT') {
                                    return (
                                        <div key={message.id} id={`appointment-${message.id}`} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} transition-all duration-500 rounded-xl`}>
                                            <AppointmentCard
                                                appointment={{
                                                    ...message,
                                                    id: message.id,
                    ...
                    < div className="bg-surface border-t border-surface-highlight p-2 sm:p-4 shrink-0" >
                        <div className="max-w-4xl mx-auto">
                            <form onSubmit={handleSendMessage} className="flex items-end gap-1 sm:gap-2">
                                ...
                                <div className="flex-1 bg-background border border-surface-highlight rounded-xl flex items-center px-4 relative">
                                    <input
                                        ...
                                        placeholder={chat?.vehicle?.status === 'ACTIVE' ? t('messages.write_message') : 'Chat bloqueado: El anuncio está desactivado'}
                            ...
                            {chat && chat.vehicle?.status !== 'ACTIVE' && (
                                <div className="mt-3 p-4 bg-red-950/20 border-2 border-red-500/30 rounded-2xl text-center shadow-lg backdrop-blur-sm animate-pulse">
                                    <div className="flex flex-col items-center gap-2">
                                        <svg className="w-8 h-8 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                        </svg>
                                        <div>
                                            <p className="text-sm text-red-400 font-black uppercase tracking-widest">
                                                Conversación Bloqueada
                                            </p>
                                            <p className="text-[10px] text-red-300 opacity-80 mt-1">
                                                El vehículo ha sido desactivado. No se pueden enviar mensajes hasta que el vendedor lo active de nuevo.
                                            </p>
                                        </div>
                                    </div>
                                </div>
