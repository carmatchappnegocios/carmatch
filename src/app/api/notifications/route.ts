...
// ... (GET function here, but we target the import first)
...
async function generateSimulatedNotifications(userId: string) {
    try {
        ...
        for (const vehicle of recentVehicles) {
            ...
            // Usamos upsertNotification que ya maneja la comprobación de duplicados (agregación)
            await upsertNotification({
                ...
                link: `/profile?vehicleId=${vehicle.id}`,
            ...
            console.log(`[SIMULATION] Notificación procesada para vehículo ${vehicle.id}`)
        }
    } catch (error) {
