...
export default function PublishClient() {
    ...
    // Computed images array for backward compatibility and internal logic
    ...
    useEffect(() => {
        ...
        batteryCapacity, range, weight, axles, status,
        latitude, longitude, city, stateLocation, countryLocation,
        selectedFeatures, userEditedFields, coverImage, galleryImages
    ])
    ...
    useEffect(() => {
        ...
        if (savedDraft) {
            try {
                ...
                // Restaurar imágenes: manejar tanto nuevo formato como antiguo 'images' para migración suave
                if (data.coverImage) setCoverImage(data.coverImage)
                if (data.galleryImages) setGalleryImages(data.galleryImages)
                
                if (!data.coverImage && !data.galleryImages && data.images && data.images.length > 0) {
    ...
    const canProceedFromStep1 = coverImage !== null
    ...
    const validateImagesAndProceed = async (retryCount = 0) => {
        if (!coverImage && galleryImages.length === 0) return
        setIsAnalyzing(true)
        setAiError('')
        ...
        try {
            ...
            if (isCoverInvalid) {
                ...
                setAiError(`⚠️ ${reason}`)
                if (coverImage) {
                    setInvalidImageUrls(new Set([coverImage]))
                    setInvalidReasons({ [coverImage]: reason })
                }
    ...
    const confirmValidationProceed = () => {
        if (validImagesCache.length > 0) {
            setCoverImage(validImagesCache[0])
            setGalleryImages(validImagesCache.slice(1))
        }
        ...
        if (editId) {
            const fetchVehicle = async () => {
                ...
                try {
                    ...
                    setCoverImage(vImages[0] || null)
                    setGalleryImages(vImages.slice(1))
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            if (!coverImage) errors.push('La foto de portada')
    ...
    return (
        <div ref={topRef} className="bg-background pb-safe">
            ...
            <div className="container mx-auto px-4 py-8 pb-32 max-w-4xl">
                ...
                <div className="bg-surface border border-surface-highlight rounded-2xl p-6 md:p-8 shadow-xl relative min-h-[300px]">
                    {isAnalyzing && (
            ...
            {/* Modal de límite de publicaciones eliminado */}
        </div>
    )
}
