...
export default function ProfileClient({ user, isOwner, vehiclesToShow }: ProfileClientProps) {
    ...
    // 🎯 Lógica para destacar un vehículo específico (por notificación)
    useEffect(() => {
        const vehicleId = searchParams.get('vehicleId')
        if (vehicleId && isOwner) {
            const index = vehiclesToShow.findIndex(v => v.id === vehicleId)
            if (index !== -1) {
                ...
                // Esperar a que la página cambie y el elemento se renderice
                setTimeout(() => {
                    const element = document.getElementById(`vehicle-${vehicleId}`)
                    if (element) {
                        element.scrollIntoView({ behavior: 'smooth', block: 'center' })
                        element.classList.add('ring-4', 'ring-primary-500', 'ring-offset-4', 'ring-offset-background', 'animate-pulse')
                        setTimeout(() => element.classList.remove('animate-pulse'), 3000)
                    }
                }, 600)
            }
        }
    }, [searchParams, vehiclesToShow, isOwner, currentPage])

    // Scroll al inicio cuando cambia la página (solo si no estamos destacando algo)
    useEffect(() => {
        const vehicleId = searchParams.get('vehicleId')
        if (!vehicleId && (currentPage > 1 || vehiclesToShow.length > ITEMS_PER_PAGE)) {
                                ...
                                {paginatedVehicles.map((vehicle) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`border rounded-xl p-5 transition-all duration-500 group relative ${isInactive
                                                ? "border-surface bg-background/30 opacity-60"
                                                : "border-surface-highlight bg-background/50 hover:border-primary-700/50"
                                                } ${searchParams.get('vehicleId') === vehicle.id ? 'ring-2 ring-primary-500 shadow-2xl' : ''}`}
                                        >
